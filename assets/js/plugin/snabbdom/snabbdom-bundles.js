...
function addEventListener_dom(ele, event, fn) {
  if (ele.attachEvent) {
    ele.attachEvent('on' + event, fn);
  } else if (ele.addEventListener_dom) {
    ele.addEventListener_dom(event, fn, false);
  }
}

function removeEventListener_dom(ele, event, fn) {
  if (ele.detachEvent) {
    ele.detachEvent('on' + event, fn);
  } else if (ele.removeEventListener_dom) {
    ele.removeEventListener_dom(event, fn, false);
  }
}
var isModule = {
	array: function(s) {
...
(function(StyleModule) {
	...
	function applyRemoveStyle(vnode, rm) {
		...
		addEventListener_dom(elm, 'transitionend', function(ev) {
		...
		if (oldOn && oldListener) {
			if (!on) {
				...
				for (name in oldOn) {
					...
					removeEventListener_dom(oldElm, name, oldListener)
				}
			} else {
				//删除旧节点中新节点不存在的事件监听
				for (name in oldOn) {
		...
		if (on) {
			...
			if (!oldOn) {
				for (name in on) {
					...
					addEventListener_dom(elm, name, listener);
				}
			} else {
				for (name in on) {
