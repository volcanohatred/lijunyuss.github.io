...
define(function (require, exports) {
	...
	var me = { // me为当前控件的函数命名空间
		...
		initClientOcr: function (widget, panel, app, json, ocrType, callback) {
			...
			if (YT.isEmpty(func) || YT.isEmpty(YT.Client[func])) { // ocr类型未定义
			...
			widget.on('click', '.x-item-ocr', function () {
				YT.Client && YT.Client[func] && YT.Client[func](ocrResultFunc); // 执行对应的插件库内容
			});
		},
		/**
		 * 解析ocr原生扫描的银行卡信息
		...
		initWebOcr: function (widget, panel, app, json, ocrType, callback) {
			...
			widget.on('change', '.webOcrInput', function (e) {
				// web接口请求参数
				var params = {};
				YT.Client && YT.Client[func] && YT.Client[func](ocrResultFunc, e, params); // 执行对应的插件库内容
			});
		}
	};
