...
//							YT.log.debug("[" + ajax.options.url + "]\n rsp:" + rspText);
							//if (YT.Client.isMessageEncrypt) {// 是否通讯解密
							if (YT.Client.encryptType != 0) {// 是否通讯解密
								...
								if (YT.Client.isGateWay) {// 是否网关通讯
									...
									if (YT.isObject(_body)) { // body 返回对象时，为失败交易，此时body为空对象
										...
										try {
											//var _decData = YT.Security.aesDecrypt(_rspdecrypt, aesKey);
											...
											opts.failure && opts.failure(_body);
										}
									}
								}
								/*if (_rspdecrypt
										&& _rspdecrypt.indexOf('{') == 0) {
									var _rpdata = YT.JsonEval(_rspdecrypt);
				...
				//if (YT.Client.isMessageEncrypt) {// 判断报文体加密是否加密
				if (YT.Client.encryptType != 0) {// 判断报文体加密是否加密
...
//						YT.log.debug("[" + ajax.options.url + "]\n Encrypt-req:" + aesdata);
					...
					/*seajs.use('assets/js/security/encrypt', function() {
						aesKey = YT.Security.getAesKey();
						if (YT.Client.isGateWay) {// 网关报文加密
							var aesdata = YT.Security.encrypt(json.body, aesKey);
							json.body = aesdata;
							YT.log.debug("[" + ajax.options.url + "]\n Encrypt-req:" + YT.JsonToStr(json));
							ajax.sendData(ajax, xhr, opts, YT.JsonToStr(json));// 发送请求
						} else {// 非网关通讯报文加密
							var aesdata = YT.Security.encrypt(json, aesKey);
							YT.log.debug("[" + ajax.options.url + "]\n Encrypt-req:" + aesdata);
							ajax.sendData(ajax, xhr, opts, aesdata);// 发送请求
						}
					});*/
